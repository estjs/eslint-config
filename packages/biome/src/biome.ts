...
import type { WasmNodejs } from './wasm';
...
export class Biome {
	private constructor(
		private readonly module: WasmNodejs,
	 ...
	 */
	public static async create(): Promise<Biome> {
	...
	private withFile<T>(path: string, content: string, func: (path: RomePath) => T): T {
		try {
	...
	formatContent(content: string, options: FormatContentDebugOptions): FormatDebugResult;
