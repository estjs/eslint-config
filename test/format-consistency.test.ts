...
interface FormatResult {
  ...
  changedFiles: Array<{ file: string; content: string }>;
}

async function runFormat(name: string, options: Options = {}): Promise<FormatResult> {
  const from = inputDir;
  ...
  await fs.writeFile(
    ...
    `
// @eslint-disable
import { defineConfig } from '${distPath}'
export default defineConfig(${JSON.stringify(options)})
  ...
  const changedFiles: Array<{ file: string; content: string }> = [];
  await Promise.all(
...
function extractScript(content?: string): [string | undefined, string | undefined] {
